...
soc_sev_arm <- adae_teae |>
  filter(TRT01A %in% arm_levels, AESEV %in% sev_levels) |>
  distinct(USUBJID, TRT01A, AEBODSYS, AESEV) |>
  count(TRT01A, AEBODSYS, AESEV) |>
...
pt_sev_arm <- adae_teae |>
  ...
  count(TRT01A, AEBODSYS, AEDECOD, AESEV) |>
  mutate(value = mapply(n_pct, n, arm_n[TRT01A])) |>
...
The `fr_table()` pipeline for the dplyr path (arm x severity columns with spans):
